...
from LeadManager.settings import LEAD_ROUTES, LEAD_PARSERS, LEAD_FIELDS
import LeadManager.settings
import urllib, urllib2

from LeadManager.msa_plugin.msafactory import get_lead_as_xml 
...
def post_lead(lead, consumer, callback):
    try:
        return  getattr(LeadManager.settings,LEAD_ROUTES[consumer.name], fallback_post_lead)(
                                                                                            consumer, 
                                                                                            lead,
                                                                                            callback)
    except KeyError:
